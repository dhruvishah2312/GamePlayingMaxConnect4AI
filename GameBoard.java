...
public class GameBoard 
{
	...
	public boolean insert(int column, int currentPlayer) {
		if (column > 6 || column < 0 || this.playBoard[0][column] != 0)
			return false;
		else { 
			for (int i = 5; i >= 0; i--)
				if (this.playBoard[i][column] == 0) {
					this.playBoard[i][column] = currentPlayer;
					break;
				}
			System.out.println("Played piece in column ->" +
		    		column + "<-");
		    this.printGameBoard();
			return true;
			}
	...
	public int isFinished(){
		...
		for (int row=0; row<rows; row++){
		    for (int col=0; col<columns-3; col++){
		    	if (this.playBoard[row][col] != 0 && this.playBoard[row][col] == this.playBoard[row][col+1] && this.playBoard[row][col] == this.playBoard[row][col+2] && this.playBoard[row][col] == this.playBoard[row][col+3]){
		...
		for (int row = 0; row < rows-3; row++){
		    for (int col = 0; col < columns; col++){
				if (this.playBoard[row][col] != 0 && this.playBoard[row][col] == this.playBoard[row+1][col] && this.playBoard[row][col] == this.playBoard[row+2][col] && this.playBoard[row][col] == this.playBoard[row+3][col]){
					return this.playBoard[row][col];
				}
			}
		}
		...
		for (int row = 0; row < rows-3; row++){
		    for (int col = 0; col < columns-3; col++){
				if (this.playBoard[row][col] != 0 && this.playBoard[row][col] == this.playBoard[row+1][col+1] &&this.playBoard[row][col] == this.playBoard[row+2][col+2] && this.playBoard[row][col] == this.playBoard[row+3][col+3]){
					return this.playBoard[row][col];
				}
			}
		}
